"""
Init-Inject Architecture untuk SimpleRNN dari nol.
Arsitektur alternatif di mana image feature digabungkan SETELAH RNN
...
# ============================================================================
# RNN Init-Inject Model
# ============================================================================

class RNNInitInject:
    """
    RNN Decoder dengan Init-Inject Architecture.

    Perbedaan dengan Pre-Inject:
        Pre-Inject:  CNN_feature → Dense → x_{-1} → di-prepend ke sequence
                     Input: [x_{-1}, emb_0, emb_1, ..., emb_{T-1}]
        Init-Inject: RNN memproses sequence embedding TERLEBIH DAHULU
                    SETELAH RNN selesai, output hidden state di-concatenate
                    dengan CNN_feature → Dense → softmax

    Arsitektur Init-Inject:
        1. x_t = Embedding(token_t) (embed_dim)
        2. RNN forward: h_T = RNN(x_0, x_1, ..., x_{T-1}) — TANPA CNN_feature
    ...
    def forward(self, cnn_features, token_seq, training=True):
        ...
        # Step 2: RNN forward — TANPA CNN feature di sini
        if self.num_layers == 1:
            self.rnn.reset_cache()
            h_out, _ = self.rnn.forward_sequence(embedded)
        else:
            h_out, _ = self.rnn.forward_sequence(embedded)
    ...
    def load_weights_from_h5(self, h5_path):
        ...
        Keras weight format untuk SimpleRNN:
            kernel: (input_dim, hidden_dim) — ini W_xh (transpose)
            recurrent_kernel: (hidden_dim, hidden_dim) — ini W_hh (transpose)
        ...
        for key, weights in sorted(layer_weights.items()):
            ...
            if 'kernel' in w_keys and 'recurrent_kernel' in w_keys:
                ...
                if kernel.shape[0] == 2 * self.hidden_dim:
                    ...
                    # Ini CNN projection: feature_dim → hidden_dim
                    W_proj = kernel.T
                    b_proj = bias
    ...
    def summary(self):
        ...
        print("RNNInitInject Architecture (Init-Inject — Tanti et al., 2017):")
        print("=" * 60)
        ...
        print("=" * 60)
        print("  Arsitektur:")
        print("    1. Embedding(token_seq) → RNN → h_T")
...
def build_initinject_from_config(vocab_size, embed_dim, hidden_dim, num_layers,
                                 feature_dim=2048):
    """
    Helper: bangun RNNInitInject model dari konfigurasi.
...
def compare_preinject_vs_initinject(preinject_model, initinject_model,
                                    cnn_features, idx2word,
                                    max_length=30, n_samples=10):
    """
    Bandingkan caption hasil Pre-Inject vs Init-Inject (RNN).

    Args:
        preinject_model: RNNScratch instance
        initinject_model: RNNInitInject instance
...
if __name__ == '__main__':
    ...
    model = RNNInitInject(vocab_size, embed_dim, hidden_dim, 1, feature_dim)
    model.build()
    model.summary()
